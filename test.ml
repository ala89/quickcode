...
let () =
  let a = choose_one_trace (generate_program [
    ([|"B45AACabcdef94VX"|], "abcdef");
    ([|"CN4toto88"|], "toto");
    ([|"desminuscules"|], "desminuscules")
  ]) in 
  assert (exec_trace [|"B45AACabcdef94VX"|] a = "abcdef");
  assert (exec_trace [|"CN4toto88"|] a = "toto");
  assert (exec_trace [|"test"|] a = "test");
  assert (exec_trace [|"PLOPcoucouAUREVOIR"|] a = "coucou");
  ...
  let c = choose_one_trace (generate_program [
    ([|"Alex"; "Asst."|], "Alex(Asst.)");
    ([|"Jim Bro"; "Manager"|], "Jim Bro(Manager)")
  ]) in
  ...
  let d = choose_one_trace (generate_program [
    ([|"Erana Yahavi"|], "Yahavi, E.");
    ([|"Bill Gates"|], "Gates, B.")
  ]) in
